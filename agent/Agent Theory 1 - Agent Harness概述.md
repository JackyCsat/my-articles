...
+ $ x_{<t} = (x_1, x_2, \dots, x_{t-1}) $：当前输出中已经生成的词元序列。
+ $ x_t $：即将生成的下一个词元。
...
# 五、Agent平台与Agent框架
Agent在传统确定性系统中引入自主决策引擎。Agent托付给集中式平台还是内嵌在业务应用，本质回答：系统决策控制流与业务数据应当如何相处？

+ 集中式平台（类似GPU）：标准化基建；开箱即用观测；团队探索期敏捷试错
+ 应用内嵌Agent框架（类似TPU）：代码开发周期长、工作量大，以此为代价换取决策速度、上下文零拷贝、容灾兜底的极致掌控
