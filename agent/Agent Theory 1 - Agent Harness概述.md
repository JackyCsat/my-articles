...
## 4.1 multiAgent
当复杂任务可以拆解为职责清晰、相对解耦的子任务时，适合采用 Multi-agent 架构。
...
+ **集中式部署**：适合供多个应用或团队共用的 Agent，如知识检索、文档分析等。任务接口清晰、与单个业务的内部状态耦合较低时，便于统一维护、观测和控成本。
+ **应用内嵌**：适合深度依赖业务状态、权限和内部逻辑的场景，便于集成与定制，但工程和维护责任更多由业务团队承担。
+ **组合使用**：模型接入、观测等通用能力集中提供，贴近业务的编排和规则留在应用内。
